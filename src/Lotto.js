...
class Lotto {
  ...
  #validate(numbers) {
    ...
    if (numbers.length !== TOTAL_LENGTH) throw new LottoError(LENGTH);

    const duplicatedNumber = new Set(numbers);
    if (duplicatedNumber.size !== numbers.length) throw new LottoError(DUPLICATION);

    const isOutOfRange = numbers.some((number) => number < START || number > END);
    if (isOutOfRange) throw new LottoError(OUT_OF_RANGE);

    const hasNaN = numbers.some((number) => isNaN(number));
    if (hasNaN) throw new LottoError(HAS_NAN);

    const isNotInteger = numbers.some((number) => !Number.isInteger(number));
    if (isNotInteger) throw new LottoError(NOT_INTEGER);
  }
