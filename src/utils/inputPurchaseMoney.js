...
function validateMoney(money) {
  if (isNaN(money)) throw new LottoError(IS_NUMBER);
  if (!Number.isInteger(money)) throw new LottoError(IS_INTEGER);
  if (money <= ZERO) throw new LottoError(ONLY_POSITIVE_NUMBER);
  if (money % LOTTO_PRICE !== ZERO) throw new LottoError(INVALID__AMOUNT_UNIT);
}
